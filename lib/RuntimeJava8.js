  ...
  class RuntimeJava8 extends S.classes.Runtime {
      ...
      return new BbPromise(function (resolve, reject) {
        ...
        switch (func.buildTool) {
          case 'maven':
            ...
            S.utils.sDebug(`"${stage} - ${region} - ${func.getName()}": Unzip cmd: ${unzipCommand}`);
            child = exec(unzipCommand,
          ...
          case 'gradle':
            ...
            const copyCommand = `cp -R ${packageRoot} ${pathDist}`
            S.utils.sDebug(`"${stage} - ${region} - ${func.getName()}": Copy cmd: ${copyCommand}`);
            // return fse.copyAsync(packageRoot, pathDist, {
            //   // filter: _this._processExcludePatterns(func, pathDist, stage, region),
            //   dereference: true
            // });
            child = exec(copyCommand,
              { stdio: [0, 1, 2], cwd: funcPath },
              (error, stdout, stderr) => {
                SCli.log(`-----------------`);
                // Show error
    ...
    build(func, stage, region) {
      ...
      return this.buildJars(func, stage, region)
        ...
        .then(function () {
          S.utils.sDebug(`"${stage} - ${region} - ${func.getName()}": Copy/Unzip complete`);
          return pathDist;
        });
    }
