  ...
  class RuntimeJava8 extends S.classes.Runtime {
    ...
    templatePrompts(func) {
      ...
      return cliPlugin.cliPromptSelect(`Please, select a supported build tool`, choices, false)
        .then(values => that.templateType = values[0].value);
    }
    ...
    scaffold(func) {
      ...
      return new BbPromise(function (resolve, reject) {
        ...
        if (that.templateType === 'maven') {
          mavenTemplates.map(function (filename) {
            return new BbPromise(function (resolve, reject) {
              let f = path.join(S.getServerlessPath(), 'templates', 'java8', that.templateType, filename);
              fs.readFileAsync(f)
                .then(function (tpl) {
                  let content = _.template(tpl)(ctx);
        ...
        } else if (that.templateType === 'gradle') {
          ...
          gradleFiles.map(function (filename) {
            return new BbPromise(function (resolve, reject) {
              let inFile = path.join(S.getServerlessPath(), 'templates', 'java8', that.templateType, filename);
              let outFile = path.join(func.getRootPath(), filename);
