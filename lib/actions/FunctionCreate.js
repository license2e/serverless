  ...
  class FunctionCreate extends S.classes.Plugin {
    ...
    functionCreate(evt) {
      ...
      return _this._prompt()
        .bind(_this)
        .then(_this._validateAndPrepareTemplate)
    ...
    _prompt() {
      ...
      return BbPromise
        ...
        .then(_this._validateAndPrepare.bind(_this))

        .then(function () {

          let runtime = S.getRuntime(_this.evt.options.runtime);
          const handler = runtime.getHandlerName ? runtime.getHandlerName(_this.evt.options.path, _this.functionName) : null,
                filePath = S.getProject().getRootPath(_this.evt.options.path, 's-function.json');
          let funcData = {
            name: _this.functionName,
            runtime: _this.evt.options.runtime,
            handler : handler
          };

          _this.function = new S.classes.Function(funcData, filePath);
        ...
        .then(function () {
          ...
          return _this.function.templatePrompts();

        })

        .then(function() {

          // Get the build tool if applicable
          _this.function.getBuildTool();
    ...
    _validateAndPrepare() {
      ...
      const relativePath = process.cwd().replace( S.getProject().getRootPath() + path.sep, '');
      ...
      _this.evt.options.path =  path.join(relativePath, _this.functionName);
    ...
    /**
     * Validate and prepare template data before creating module
     */

    _validateAndPrepareTemplate() {
